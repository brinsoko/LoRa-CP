...
def _parse_card_payload(
    payload: dict, require_team: bool = True
...
def rfid_card_list():
    ...
    cards = (
        RFIDCard.query.join(Team, RFIDCard.team_id == Team.id)
...
def rfid_card_create():
    ...
    team = (
        Team.query.filter(Team.competition_id == comp_id, Team.id == team_id).first() if team_id is not None else None
    )
...
def _update_card(card_id: int, partial: bool):
    ...
    if team_id is not None and not Team.query.filter(Team.competition_id == comp_id, Team.id == team_id).first():
        return jsonify({"error": "validation_error", "detail": _("Invalid team_id")}), 400

    if team_id is not None:
        exists = RFIDCard.query.filter(RFIDCard.team_id == team_id, RFIDCard.id != card.id).first()
        if exists:
            return jsonify(
                {
                    "error": "conflict",
...
def rfid_bulk_import():
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if team_id_val:
            ...
            team = Team.query.filter(Team.competition_id == comp_id, Team.name.ilike(team_name)).first()
            if team:
                team_id = team.id
        if team_id and not Team.query.filter(Team.competition_id == comp_id, Team.id == team_id).first():
            skipped += 1
        ...
        if team_id:
            with db.session.no_autoflush:
                conflict = RFIDCard.query.filter(RFIDCard.team_id == team_id, RFIDCard.id != card.id).first()
            if conflict:
                skipped += 1
...
def rfid_verify():
    ...
    if checkpoint_ids is not None:
        checkpoints = Checkpoint.query.filter(
            Checkpoint.competition_id == comp_id, Checkpoint.id.in_(checkpoint_ids)
        ).all()
        ...
        device_lookup = {
            d.dev_num: d
            for d in LoRaDevice.query.filter(
                LoRaDevice.competition_id == comp_id, LoRaDevice.dev_num.in_(device_ids)
            ).all()
        ...
        checkpoint_lookup = {
            c.id: c
            for c in Checkpoint.query.filter(
                Checkpoint.competition_id == comp_id, Checkpoint.lora_device_id.in_(device_ids_for_cp)
            ).all()
        }
    elif allowed_checkpoint_ids:
        checkpoints = Checkpoint.query.filter(
            Checkpoint.competition_id == comp_id, Checkpoint.id.in_(allowed_checkpoint_ids)
        ).all()
        checkpoint_lookup = {c.id: c for c in checkpoints}
        for cp in checkpoints:
            if cp.lora_device and cp.lora_device.dev_num is not None:
        ...
        checkpoints = Checkpoint.query.filter(Checkpoint.competition_id == comp_id).all()
        checkpoint_lookup = {c.id: c for c in checkpoints}
        device_lookup = {
            d.dev_num: d
            for d in LoRaDevice.query.filter(LoRaDevice.competition_id == comp_id).all()
            if d.dev_num is not None
        }
